...
# Part II — the extension questions (101–200)

Questions 101–200 cover **multilingual NLP**, **multimodality** and **machine translation**.
They introduce a second vocabulary, and it leans on different corners of the games: regional
forms rather than type match-ups, the Pokédex-as-instrument rather than the Pokémon, and a
great deal of the overworld — the Poké Mart, the Day Care, the Ruins of Alph.

Everything below actually appears in `answers/pokemon/101–200`. As in Part I, the last column
says what the thing is standing in for, not merely what it is.

## The spine of Part II

Six ideas carry most of the extension. If you read only one table, read this one.

| Thing | What it is in the games | What it stands for |
| --- | --- | --- |
| **Regional forms** — Alolan **Vulpix**, Galarian **Ponyta**, Alolan **Raichu** | The same species, evolved differently in a different region: Kantonian Vulpix is Fire, Alolan Vulpix is **Ice**. Both are correct Vulpix | **A language variety, not an error.** A Kanto-built Pokédex reporting an Alolan Vulpix as "malformed" is a tool announcing where it was built ([`102`](answers/pokemon/102-tokenizer-fairness-token-premium.md), [`103`](answers/pokemon/103-curse-of-multilinguality.md), [`105`](answers/pokemon/105-shared-multilingual-vocabulary.md), [`106`](answers/pokemon/106-script-vs-language.md), [`110`](answers/pokemon/110-language-adapters.md), [`113`](answers/pokemon/113-morphology-rich-languages.md), +12 more). Also the off-target failure — asking for one form and getting the dominant one ([`106`](answers/pokemon/106-script-vs-language.md), [`110`](answers/pokemon/110-language-adapters.md), [`116`](answers/pokemon/116-multilingual-instruction-tuning.md), [`136`](answers/pokemon/136-off-target-and-hallucinated-translation.md), [`162`](answers/pokemon/162-dialects-and-varieties.md), [`168`](answers/pokemon/168-multimodal-embeddings.md), +3 more) |
| **The Pokédex itself** | Point it at a Pokémon, it names the species and reads an entry | **The vision-language model.** Part II treats it as a two-part device — a lens that cannot speak and a voice that has never seen anything — and most of the multimodal questions are about the wire between them |
| **Unown** | 28 shapes, one per letter, that live in the Ruins of Alph | **An alphabet, and the tokenizer's problem.** A script the machine may have no font for, no keyboard for, and no vocabulary entry for |
| **Ditto** | Transform copies the target's species, stats and moves — **but not its HP** | Two jobs. **Back-translation**: you have no partner, so you manufacture one, and the offspring is real even though the parent was not. And **the watermark**: the HP it keeps is a mark that survives the disguise |
| **Original Trainer** | Every Pokémon permanently records who caught it, where, and when | **Provenance.** Trade it round the world and the record survives — the standard that C2PA-style content credentials aim at, and the attribution standard for community-collected data |
| **Seen vs caught** | The Pokédex shows two counters, and nobody confuses them | **A coverage claim.** "Supports 200 languages" is the *seen* number printed as though it were the caught one. The dataset's closing argument |

## Species named in 101–200

| Species | Type | What it is | What it stands for here |
| --- | --- | --- | --- |
| **Vulpix / Ninetales** | Fire (Kanto) · Ice (Alola) | A fox with six tails, and its nine-tailed evolution. **Alola's is Ice.** | The dataset's regional-variety workhorse — dialects that are not errors, and off-target output that returns the dominant form ([`103`](answers/pokemon/103-curse-of-multilinguality.md), [`104`](answers/pokemon/104-language-sampling-pretraining.md), [`106`](answers/pokemon/106-script-vs-language.md), [`111`](answers/pokemon/111-cross-lingual-embedding-alignment.md), [`136`](answers/pokemon/136-off-target-and-hallucinated-translation.md), [`150`](answers/pokemon/150-localisation-beyond-text.md), +8 more) |
| **Larvitar** | Rock/Ground | A small green pupa that eventually becomes Tyranitar | A concrete *thing being talked about* in multilingual examples — the word a sentence is about, when what matters is the sentence's language rather than its content ([`104`](answers/pokemon/104-language-sampling-pretraining.md), [`108`](answers/pokemon/108-language-identification.md), [`178`](answers/pokemon/178-translating-user-generated-content.md)) |
| **Porygon** | Normal | A Pokémon made entirely of programmable code, built by Silph Co., able to move between the physical and the digital | **The connector.** The thing that converts what the lens saw into something the voice can read ([`117`](answers/pokemon/117-vision-language-architectures.md), [`135`](answers/pokemon/135-llm-versus-nmt-translation.md)) |
| **Porygon2 / Porygon-Z** | Normal | Porygon upgraded twice. Porygon2 (Up-Grade) is stable; **Porygon-Z** (Dubious Disc) hits harder and the Pokédex says outright that its programming was altered and it now behaves oddly | **Dedicated NMT versus a large language model.** Porygon2 is cheap, fast and predictable; Porygon-Z is more capable and less reliable, and the games say so on the tin ([`112`](answers/pokemon/112-byte-level-and-tokenizer-free.md), [`115`](answers/pokemon/115-unicode-normalisation.md), [`135`](answers/pokemon/135-llm-versus-nmt-translation.md), [`180`](answers/pokemon/180-mt-infrastructure-and-cost.md), [`194`](answers/pokemon/194-on-device-translation.md), [`200`](answers/pokemon/200-what-actually-matters.md)) |
| **Unown** | Psychic | 28 forms, one per letter of an alphabet, released by solving the Ruins of Alph slate puzzles | Scripts, alphabets, and text that has been chopped into symbols ([`101`](answers/pokemon/101-cross-lingual-transfer.md), [`102`](answers/pokemon/102-tokenizer-fairness-token-premium.md), [`105`](answers/pokemon/105-shared-multilingual-vocabulary.md), [`108`](answers/pokemon/108-language-identification.md), [`109`](answers/pokemon/109-transliteration-romanisation.md), [`112`](answers/pokemon/112-byte-level-and-tokenizer-free.md), +6 more) |
| **Rotom** | Electric/Ghost | A Pokémon that possesses appliances, changing type with each one. The **Rotom Pokédex** rides along with the Trainer all day | Two jobs: **domain switching** — one creature, five specialisms, swapped at the door — and **the wearable camera** that sees everything you see ([`103`](answers/pokemon/103-curse-of-multilinguality.md), [`110`](answers/pokemon/110-language-adapters.md), [`114`](answers/pokemon/114-word-segmentation-no-spaces.md), [`115`](answers/pokemon/115-unicode-normalisation.md), [`134`](answers/pokemon/134-domain-adaptation-translation.md), [`174`](answers/pokemon/174-egocentric-and-streaming-perception.md), +1 more) |
| **Silvally** | Normal, or whatever disc it holds | Built artificially; a **memory disc** changes its type | **Adapters.** One frozen base, small swappable pieces, one per language ([`103`](answers/pokemon/103-curse-of-multilinguality.md), [`110`](answers/pokemon/110-language-adapters.md), [`194`](answers/pokemon/194-on-device-translation.md)) |
| **Voltorb / Electrode** | Electric | Looks **exactly like a Poké Ball**, sits among the items on the floor in Silph Co. and the Rocket Hideout, and Self-Destructs | **An attack that arrives shaped like the thing you trust.** The whole of image-borne prompt injection ([`108`](answers/pokemon/108-language-identification.md), [`137`](answers/pokemon/137-diffusion-image-generation.md), [`139`](answers/pokemon/139-multimodal-safety-attacks.md), [`169`](answers/pokemon/169-mt-security-and-poisoning.md), [`178`](answers/pokemon/178-translating-user-generated-content.md), [`192`](answers/pokemon/192-multimodal-monitoring.md), +2 more) |
| **Mewtwo** | Psychic | Genetically engineered, one of the strongest Pokémon in the games | **The frontier model.** Excellent, and not something you send out thirty thousand times a day ([`180`](answers/pokemon/180-mt-infrastructure-and-cost.md), [`182`](answers/pokemon/182-vlm-distillation.md), [`185`](answers/pokemon/185-massively-multilingual-models.md), [`199`](answers/pokemon/199-choosing-an-approach.md)) |
| **Combee / Vespiquen** | Bug/Flying | Combee are about seven-eighths male, and **only a female Combee ever evolves** into Vespiquen | **A prior overriding the individual.** Right seven times in eight, and the eighth Trainer loses their Vespiquen ([`142`](answers/pokemon/142-gender-bias-in-translation.md)) |
| **Machamp** | Fighting | Four arms | Doing several things simultaneously — holding two references at once, or a body whose skills do not transfer to a two-armed one ([`116`](answers/pokemon/116-multilingual-instruction-tuning.md), [`155`](answers/pokemon/155-sign-language-translation.md), [`157`](answers/pokemon/157-video-generation.md), [`181`](answers/pokemon/181-vision-language-action.md)) |
| **Wailord** | Water | 14.5 m long, and the largest Pokémon by length | **Aspect ratio and scale.** Nothing you actually want to look at is square, and forcing Wailord and Diglett into the same frame distorts both ([`106`](answers/pokemon/106-script-vs-language.md), [`111`](answers/pokemon/111-cross-lingual-embedding-alignment.md), [`121`](answers/pokemon/121-high-resolution-tiling.md), [`131`](answers/pokemon/131-document-level-translation.md), [`150`](answers/pokemon/150-localisation-beyond-text.md), [`159`](answers/pokemon/159-referring-segmentation.md)) |
| **Whismur / Loudred / Exploud** | Normal | A line defined entirely by volume | Audio: the low rumble end of the pitch ladder, and background noise in a recording ([`114`](answers/pokemon/114-word-segmentation-no-spaces.md), [`125`](answers/pokemon/125-speech-recognition.md), [`138`](answers/pokemon/138-speech-language-models.md), [`173`](answers/pokemon/173-audio-and-music-generation.md)) |
| **Meowth** | Normal | The Team Rocket Meowth taught itself to speak human language | **The cascade.** Convert the cry into words, think in words, speak the answer — and lose everything that was not the words ([`109`](answers/pokemon/109-transliteration-romanisation.md), [`138`](answers/pokemon/138-speech-language-models.md), [`154`](answers/pokemon/154-dubbing-and-lip-sync.md)) |
| **Kabuto / Omanyte / Aerodactyl** | Rock/Water · Rock/Water · Rock/Flying | Extinct species revived from the Dome Fossil, Helix Fossil and Old Amber. Their images are the slate puzzles in the Ruins of Alph | Species known only from fragments — and, as puzzles, **an image cut into tiles** ([`109`](answers/pokemon/109-transliteration-romanisation.md), [`118`](answers/pokemon/118-image-patches-tokenisation.md), [`140`](answers/pokemon/140-multimodal-retrieval.md), [`165`](answers/pokemon/165-vlm-inference-efficiency.md)) |
| **Chatot** | Normal/Flying | Mimics what it hears; its signature move **Chatter** recorded the Trainer's own voice and played it back as an attack | **Voice cloning, and hallucination on silence.** A machine whose whole nature is producing plausible output will produce it whether or not anything was said ([`114`](answers/pokemon/114-word-segmentation-no-spaces.md), [`125`](answers/pokemon/125-speech-recognition.md), [`138`](answers/pokemon/138-speech-language-models.md), [`153`](answers/pokemon/153-speech-to-speech-translation.md)) |
| **Diglett / Dugtrio** | Ground | 0.2 m. Dugtrio is **three Digletts** | Scale at the small end, and the **part-versus-whole** problem: ask for "the Diglett" in a Dugtrio and there is no single right answer ([`121`](answers/pokemon/121-high-resolution-tiling.md), [`152`](answers/pokemon/152-3d-and-depth-understanding.md), [`157`](answers/pokemon/157-video-generation.md), [`159`](answers/pokemon/159-referring-segmentation.md)) |
| **Mimikyu** | Ghost/Fairy | Wears a crude rag drawn to look like Pikachu. Its Disguise ability blocks one hit | **An attack that does not have to convince you, only the machine.** The rag fools nobody and works anyway ([`108`](answers/pokemon/108-language-identification.md), [`115`](answers/pokemon/115-unicode-normalisation.md), [`120`](answers/pokemon/120-contrastive-image-text-pretraining.md), [`139`](answers/pokemon/139-multimodal-safety-attacks.md), [`155`](answers/pokemon/155-sign-language-translation.md), [`169`](answers/pokemon/169-mt-security-and-poisoning.md)) |
| **Farfetch'd / Sirfetch'd** | Normal/Flying · Fighting | A duck carrying a leek, and its evolution. **Both names contain an apostrophe** | Unicode and text normalisation: names whose punctuation is part of the name ([`112`](answers/pokemon/112-byte-level-and-tokenizer-free.md), [`115`](answers/pokemon/115-unicode-normalisation.md)) |
| **Mr. Mime / Mime Jr.** | Psychic/Fairy | Names containing a full stop, a space, and in one case both | The same problem, worse — and a title that must not be translated ([`101`](answers/pokemon/101-cross-lingual-transfer.md), [`112`](answers/pokemon/112-byte-level-and-tokenizer-free.md), [`115`](answers/pokemon/115-unicode-normalisation.md), [`172`](answers/pokemon/172-named-entity-translation.md)) |
| **Pidgey / Pidgeot** | Normal/Flying | The generic early-route bird | **The common case.** The thing your archive is full of, and the cheap request that never needed the expensive model ([`104`](answers/pokemon/104-language-sampling-pretraining.md), [`108`](answers/pokemon/108-language-identification.md), [`122`](answers/pokemon/122-multimodal-hallucination.md), [`124`](answers/pokemon/124-interleaved-multimodal-data.md), [`127`](answers/pokemon/127-chart-and-diagram-reasoning.md), [`148`](answers/pokemon/148-modality-balance-training-recipes.md), +2 more) |
| **Shuckle** | Bug/Rock | Base Defence and Special Defence of **230** each, and Attack of 10 | A genuine stat outlier, used against Blissey to make the point that there is no best Pokémon — only best at something ([`127`](answers/pokemon/127-chart-and-diagram-reasoning.md), [`160`](answers/pokemon/160-multimodal-chain-of-thought.md), [`182`](answers/pokemon/182-vlm-distillation.md)) |
| **Togekiss / Amoonguss** | Fairy/Flying · Grass/Poison | The two standard **redirection** Pokémon in Doubles, via Follow Me and Rage Powder | **Attention sinks and register tokens** — a designated volunteer that absorbs the targeting that would otherwise land somewhere meaningless ([`119`](answers/pokemon/119-cross-modal-attention.md), [`160`](answers/pokemon/160-multimodal-chain-of-thought.md)) |
| **Fletchinder · Crabominable · Blacephalon · Centiskorch** | various | Species whose names are eleven and twelve characters long | **Length constraints.** The nickname cap was ten characters, and was raised to twelve because the names outgrew the box ([`144`](answers/pokemon/144-length-constrained-translation.md), [`150`](answers/pokemon/150-localisation-beyond-text.md)) |
| **Jigglypuff** | Normal/Fairy | Sings, and puts everything to sleep | Audio generation: **Sing either works or it does not**, and a Jigglypuff nearly in tune has simply failed ([`168`](answers/pokemon/168-multimodal-embeddings.md), [`173`](answers/pokemon/173-audio-and-music-generation.md)) |
| **Joltik** | Bug/Electric | 0.1 m — the smallest Pokémon there is | A tiny finding in an enormous image ([`151`](answers/pokemon/151-medical-scientific-imaging.md), [`153`](answers/pokemon/153-speech-to-speech-translation.md), [`159`](answers/pokemon/159-referring-segmentation.md)) |
| **Nidoran♀ / Nidoran♂** | Poison | **Two separate species** distinguished only by a symbol. ♀ becomes Nidorina then Nidoqueen; ♂ becomes Nidorino then Nidoking | **Pronoun and gender resolution.** The information is one line up, most languages must mark it, and getting it wrong writes Nidoking into the Nidoqueen entry ([`104`](answers/pokemon/104-language-sampling-pretraining.md), [`108`](answers/pokemon/108-language-identification.md), [`112`](answers/pokemon/112-byte-level-and-tokenizer-free.md), [`115`](answers/pokemon/115-unicode-normalisation.md), [`117`](answers/pokemon/117-vision-language-architectures.md), [`130`](answers/pokemon/130-low-resource-translation.md), +5 more) |
| **Meowstic** | Psychic | Male and female genuinely differ — different abilities, different learnsets | Gender that is not cosmetic: blurring the two makes the entry **wrong**, not merely clumsy ([`131`](answers/pokemon/131-document-level-translation.md), [`142`](answers/pokemon/142-gender-bias-in-translation.md)) |
| **Salandit / Salazzle** | Poison/Fire | **Only female Salandit evolve** | The same gender-gated outcome as Combee ([`142`](answers/pokemon/142-gender-bias-in-translation.md)) |
| **Smeargle** | Normal | **Sketch** copies exactly one move and keeps it forever | **The resampler.** A fixed, tiny summary of an arbitrarily complicated thing — bounded cost, and everything not sketched is gone ([`112`](answers/pokemon/112-byte-level-and-tokenizer-free.md), [`117`](answers/pokemon/117-vision-language-architectures.md)) |
| **Zoroark** | Dark | **Illusion** makes it appear as the last Pokémon in the party until it takes a damaging hit | Impersonation, and a detector that only works once you attack ([`108`](answers/pokemon/108-language-identification.md), [`115`](answers/pokemon/115-unicode-normalisation.md), [`120`](answers/pokemon/120-contrastive-image-text-pretraining.md), [`184`](answers/pokemon/184-face-recognition-and-biometrics.md)) |
| **Zapdos / Moltres / Articuno** | Electric/Flying · Fire/Flying · Ice/Flying | The Kanto legendary birds | **Spatial relations.** A model names both correctly and puts them on the wrong sides of the sky. Also "seen, not caught" ([`117`](answers/pokemon/117-vision-language-architectures.md), [`120`](answers/pokemon/120-contrastive-image-text-pretraining.md), [`192`](answers/pokemon/192-multimodal-monitoring.md)) |
| **Doduo / Dodrio** | Normal/Flying | Two heads, and three | **The Janus problem** in 3D generation: you asked for a Doduo and every angle grew a face ([`191`](answers/pokemon/191-3d-scene-generation.md)) |
| **Girafarig** | Normal/Psychic | Has a **second head on its tail** | The same failure, palindromically ([`191`](answers/pokemon/191-3d-scene-generation.md)) |
| **Ekans / Arbok · Sandshrew / Sandslash · Scyther · Pinsir · Growlithe** | various | **Version exclusives** — Ekans and Scyther and Growlithe are in *Red*, Sandshrew and Pinsir and Vulpix in *Blue* | **Content that differs by locale**, not translation. You cannot translate a Growlithe into a Vulpix ([`150`](answers/pokemon/150-localisation-beyond-text.md), [`177`](answers/pokemon/177-multilingual-rag.md), [`195`](answers/pokemon/195-multilingual-reasoning.md)) |
| **Sudowoodo** | Rock | Imitates a tree. Is not a tree | Something that looks right from every angle and is not the thing — a generated model you cannot animate ([`106`](answers/pokemon/106-script-vs-language.md), [`191`](answers/pokemon/191-3d-scene-generation.md)) |
| **Deerling** | Normal/Grass | Changes colour with the **season** | **Change detection's confound.** The route looks different because the season turned, not because anything happened ([`113`](answers/pokemon/113-morphology-rich-languages.md), [`175`](answers/pokemon/175-remote-sensing-imagery.md)) |
| **Kricketune** | Bug | Known for its song | Musical structure at several timescales at once ([`125`](answers/pokemon/125-speech-recognition.md), [`173`](answers/pokemon/173-audio-and-music-generation.md)) |
| **Relicanth** | Water/Rock | A living fossil, unchanged for a hundred million years | A language isolate: no relatives left to transfer from ([`163`](answers/pokemon/163-endangered-language-documentation.md)) |
| **Blissey / Chansey / Audino** | Normal | The Pokémon that assist at Pokémon Centres. **Blissey's 255 base HP** is the highest in the games; its Attack is 10 | Nurse Joy's desk in the quality-estimation answers, and the canonical stat outlier ([`126`](answers/pokemon/126-video-understanding.md), [`127`](answers/pokemon/127-chart-and-diagram-reasoning.md), [`128`](answers/pokemon/128-visual-grounding-spatial-reasoning.md), [`132`](answers/pokemon/132-quality-estimation.md), [`140`](answers/pokemon/140-multimodal-retrieval.md), [`145`](answers/pokemon/145-gui-computer-use-agents.md), +4 more) |
| **Deoxys** | Psychic | In Attack Forme, enormous offence and paper defences | The other end of the same argument: no best Pokémon, only best at something ([`182`](answers/pokemon/182-vlm-distillation.md)) |
| **Magneton / Exeggcute** | Electric/Steel · Grass/Psychic | **Three** Magnemite; **six** eggs | Part-versus-whole, again — and counting things that are one thing ([`159`](answers/pokemon/159-referring-segmentation.md)) |
| **Wurmple / Silcoon / Cascoon** | Bug | Wurmple splits into two evolution lines **with nothing visible deciding which** | Following the wrong edge in a diagram whose branches cross ([`127`](answers/pokemon/127-chart-and-diagram-reasoning.md)) |
| **Charmeleon** | Fire | The middle Charmander stage. **It has no wings** — Charizard does | The dataset's canonical hallucination: a fluent describer writes about the wings anyway ([`122`](answers/pokemon/122-multimodal-hallucination.md), [`136`](answers/pokemon/136-off-target-and-hallucinated-translation.md), [`146`](answers/pokemon/146-synthetic-captions-data-curation.md), [`154`](answers/pokemon/154-dubbing-and-lip-sync.md), [`157`](answers/pokemon/157-video-generation.md), [`160`](answers/pokemon/160-multimodal-chain-of-thought.md), +7 more) |
| **Snorlax** | Normal | Sleeps across Routes 12 and 16 and cannot be moved without the Poké Flute | The obstacle that a tool either clears or does not — **no approximately** ([`111`](answers/pokemon/111-cross-lingual-embedding-alignment.md), [`121`](answers/pokemon/121-high-resolution-tiling.md), [`146`](answers/pokemon/146-synthetic-captions-data-curation.md), [`152`](answers/pokemon/152-3d-and-depth-understanding.md), [`153`](answers/pokemon/153-speech-to-speech-translation.md), [`165`](answers/pokemon/165-vlm-inference-efficiency.md), +3 more) |
...
| Place | What it is | What it stands for |
| --- | --- | --- |
| **Ruins of Alph** | Four sealed chambers near Violet City. Each holds a **slate puzzle** — a picture cut into square tiles, scrambled — showing Kabuto, Omanyte, Aerodactyl or Ho-Oh. Solving them releases the Unown | **Image patching**, exactly. A picture is not something a reader can read; a row of tiles is. Also the undeciphered script, and the wall of writing nobody can transcribe fast enough ([`101`](answers/pokemon/101-cross-lingual-transfer.md), [`105`](answers/pokemon/105-shared-multilingual-vocabulary.md), [`109`](answers/pokemon/109-transliteration-romanisation.md), [`118`](answers/pokemon/118-image-patches-tokenisation.md), [`121`](answers/pokemon/121-high-resolution-tiling.md), [`163`](answers/pokemon/163-endangered-language-documentation.md), +2 more) |
| **Celadon Department Store** | Six floors, a directory board by the lift, shelves with prices | **A document.** Layout, reading order, small print, and a table you must not split ([`105`](answers/pokemon/105-shared-multilingual-vocabulary.md), [`123`](answers/pokemon/123-document-understanding-ocr.md), [`139`](answers/pokemon/139-multimodal-safety-attacks.md), [`140`](answers/pokemon/140-multimodal-retrieval.md), [`141`](answers/pokemon/141-formality-and-honorifics.md), [`165`](answers/pokemon/165-vlm-inference-efficiency.md), +2 more) |
| **Kanto / Johto / Hoenn / Sinnoh / Unova / Kalos / Alola / Galar / Paldea** | The nine regions of the games | **Languages.** Kanto is the high-resource one everything is routed through, and Paldea and Alola are where the pipeline turns out to have been Kanto-centric all along ([`101`](answers/pokemon/101-cross-lingual-transfer.md), [`103`](answers/pokemon/103-curse-of-multilinguality.md), [`105`](answers/pokemon/105-shared-multilingual-vocabulary.md), [`110`](answers/pokemon/110-language-adapters.md), [`112`](answers/pokemon/112-byte-level-and-tokenizer-free.md), [`125`](answers/pokemon/125-speech-recognition.md), +5 more) |
| **Poké Mart** | The shop. Fixed items at fixed prices | Terminology that must not be translated, and prices that are a business decision rather than arithmetic ([`150`](answers/pokemon/150-localisation-beyond-text.md), [`165`](answers/pokemon/165-vlm-inference-efficiency.md), [`192`](answers/pokemon/192-multimodal-monitoring.md)) |
| **Day Care** | Leave two compatible Pokémon and receive an egg | **Where translations come from.** A bred Pokémon and a wild-caught one are both legitimate and tell apart instantly — translationese ([`170`](answers/pokemon/170-translationese.md)) |
| **Lake of Rage** | A lake containing one **red Gyarados** — a shiny, in a known place | Rare-event retrieval, and the hard negative that separates a good filing system from a rough one ([`106`](answers/pokemon/106-script-vs-language.md), [`158`](answers/pokemon/158-image-editing-inpainting.md), [`164`](answers/pokemon/164-participatory-mt.md), [`168`](answers/pokemon/168-multimodal-embeddings.md)) |
| **Silph Co.** | The Saffron City building, occupied by Team Rocket, with Voltorb disguised among the items | Where the disguised attack is on the floor ([`139`](answers/pokemon/139-multimodal-safety-attacks.md), [`169`](answers/pokemon/169-mt-security-and-poisoning.md), [`172`](answers/pokemon/172-named-entity-translation.md), [`193`](answers/pokemon/193-multilingual-safety.md)) |
| **Mt. Moon** | A cave. Full of Zubat | **Co-occurrence bias.** Show a model any cave and it will tell you about the Zubat ([`160`](answers/pokemon/160-multimodal-chain-of-thought.md), [`164`](answers/pokemon/164-participatory-mt.md), [`192`](answers/pokemon/192-multimodal-monitoring.md), [`193`](answers/pokemon/193-multilingual-safety.md)) |
| **Global Trade Station / Union Room** | Where Trainers trade and talk. The Union Room offers **preset phrases** rather than a keyboard | User-generated content — and the games' own concession that free-typed chat is a mess ([`178`](answers/pokemon/178-translating-user-generated-content.md)) |
| **Pallet Town** | Where you start, and where Professor Oak hands you an empty Pokédex | The opening and closing frame of the dataset ([`101`](answers/pokemon/101-cross-lingual-transfer.md), [`172`](answers/pokemon/172-named-entity-translation.md), [`194`](answers/pokemon/194-on-device-translation.md), [`200`](answers/pokemon/200-what-actually-matters.md)) |
| **Cerulean Cave** | Holds Mewtwo, behind a door most Trainers see and never open | "Seen, not caught" ([`185`](answers/pokemon/185-massively-multilingual-models.md)) |
| **Lavender Town** | Home of the **Name Rater** | Where names get changed — and where the rule "some names are yours to change and some are not" lives ([`115`](answers/pokemon/115-unicode-normalisation.md), [`133`](answers/pokemon/133-terminology-glossary-enforcement.md), [`178`](answers/pokemon/178-translating-user-generated-content.md)) |

## People and roles named in 101–200

| Who | What they are | What they stand for |
| --- | --- | --- |
| **Nurse Joy** | Runs the Pokémon Centre; heals what you bring in | **Quality estimation.** Judging a translation with no reference — is this fit to go back out, does it need patching, or must it be held back ([`132`](answers/pokemon/132-quality-estimation.md), [`135`](answers/pokemon/135-llm-versus-nmt-translation.md), [`136`](answers/pokemon/136-off-target-and-hallucinated-translation.md), [`141`](answers/pokemon/141-formality-and-honorifics.md), [`149`](answers/pokemon/149-post-editing-human-in-the-loop.md), [`151`](answers/pokemon/151-medical-scientific-imaging.md), +5 more) |
| **Professor Oak** | Gives you the Pokédex and asks you to fill it. **You** do all the walking | Two jobs: the **photo album** whose captions are the training signal, and the uncomfortable question of **whose research it is** when the community does the fieldwork ([`117`](answers/pokemon/117-vision-language-architectures.md), [`124`](answers/pokemon/124-interleaved-multimodal-data.md), [`140`](answers/pokemon/140-multimodal-retrieval.md), [`161`](answers/pokemon/161-translation-memory-systems.md), [`164`](answers/pokemon/164-participatory-mt.md), [`174`](answers/pokemon/174-egocentric-and-streaming-perception.md), +4 more) |
| **The Kanto Gym Leaders** — Brock, Misty, Lt. Surge, Erika, Koga, Sabrina, Blaine, Giovanni | Eight specialists, each one type, in a **canonical order** | **A pipeline with stages**, and **domain experts** — magnificent inside their hall, ordinary outside it. Also an evaluation grid: eight badges beats one average ([`101`](answers/pokemon/101-cross-lingual-transfer.md), [`134`](answers/pokemon/134-domain-adaptation-translation.md), [`198`](answers/pokemon/198-building-an-evaluation-suite.md)) |
| **The Elite Four** — Lorelei, Bruno, Agatha, Lance | Fixed teams, fixed order, faced in sequence | **A benchmark.** Beating them proves less than it looks like, because they never change ([`147`](answers/pokemon/147-multimodal-benchmarks-contamination.md), [`148`](answers/pokemon/148-modality-balance-training-recipes.md), [`171`](answers/pokemon/171-human-evaluation-translation.md)) |
| **Youngster Joey · Bug Catcher · Ace Trainer** | **Trainer Classes** — the label printed before a battle starts | **Register, declared up front.** Joey does not talk like a Gentleman, and the game tells you which you are getting ([`141`](answers/pokemon/141-formality-and-honorifics.md)) |
| **Name Rater · Move Deleter · Move Reminder** | The three NPCs who change a Pokémon after the fact | **Post-editing.** Repairing something somebody else raised, and the fact that a traded Pokémon keeps its Original Trainer through all of it ([`134`](answers/pokemon/134-domain-adaptation-translation.md), [`149`](answers/pokemon/149-post-editing-human-in-the-loop.md), [`188`](answers/pokemon/188-cat-tools-and-translator-workflow.md)) |
| **Team Rocket** | Operate under Celadon, in Mt. Moon, inside Silph Co. — never where a referee is watching | **Corpus poisoning and safety coverage gaps.** They do not break the rules; they work where the rules are not enforced ([`154`](answers/pokemon/154-dubbing-and-lip-sync.md), [`169`](answers/pokemon/169-mt-security-and-poisoning.md), [`175`](answers/pokemon/175-remote-sensing-imagery.md), [`179`](answers/pokemon/179-controlled-language-authoring.md), [`193`](answers/pokemon/193-multilingual-safety.md)) |

## Items, moves and mechanics named in 101–200

| Term | What it is | What it stands for |
| --- | --- | --- |
| **Original Trainer** | Permanently recorded on every Pokémon: who caught it, where, at what level, on what date | **Provenance and attribution.** Survives every trade. The standard content credentials aim at, and the standard for crediting community data ([`149`](answers/pokemon/149-post-editing-human-in-the-loop.md), [`161`](answers/pokemon/161-translation-memory-systems.md), [`164`](answers/pokemon/164-participatory-mt.md), [`166`](answers/pokemon/166-watermarking-and-provenance.md), [`170`](answers/pokemon/170-translationese.md), [`172`](answers/pokemon/172-named-entity-translation.md), +1 more) |
| **Type Chart** | The 18×18 grid of effectiveness multipliers | **A chart, literally** — the thing a model must read rather than recognise. Also the thing you check *before* choosing an approach ([`140`](answers/pokemon/140-multimodal-retrieval.md), [`171`](answers/pokemon/171-human-evaluation-translation.md), [`184`](answers/pokemon/184-face-recognition-and-biometrics.md), [`199`](answers/pokemon/199-choosing-an-approach.md)) |
| **Egg Group / Egg Move** | Species that can breed share an Egg Group; a parent passes moves the child could never learn alone | **Cross-lingual transfer.** Charmander gets Dragon Dance from a relative because no TM teaches it ([`130`](answers/pokemon/130-low-resource-translation.md), [`170`](answers/pokemon/170-translationese.md)) |
| **Four-Move Limit** | Four moves. Teaching a fifth **deletes** one | **Catastrophic forgetting**, stated as a hard rule rather than a tendency ([`134`](answers/pokemon/134-domain-adaptation-translation.md), [`148`](answers/pokemon/148-modality-balance-training-recipes.md)) |
| **Bottle Cap · Ability Capsule · Heart Scale · Mint · the EV-lowering berries** | The toolkit for altering a Pokémon somebody else raised | **Post-editing tools** ([`149`](answers/pokemon/149-post-editing-human-in-the-loop.md), [`188`](answers/pokemon/188-cat-tools-and-translator-workflow.md)) |
| **Protein · Iron · Calcium · Zinc · Carbos · HP Up** | The six vitamins, one per stat. **510 effort points total, 252 maximum in any one** | **The training data mixture.** A fixed budget across dimensions, and what you spent it on is what you did not ([`148`](answers/pokemon/148-modality-balance-training-recipes.md)) |
| **Full Restore vs Full Heal** | Two different items. One restores HP *and* status; the other only status | **Terminology that is the meaning.** The canonical glossary error in the translation questions ([`123`](answers/pokemon/123-document-understanding-ocr.md), [`150`](answers/pokemon/150-localisation-beyond-text.md), [`151`](answers/pokemon/151-medical-scientific-imaging.md), [`186`](answers/pokemon/186-terminology-mining.md), [`187`](answers/pokemon/187-mt-in-regulated-domains.md), [`188`](answers/pokemon/188-cat-tools-and-translator-workflow.md)) |
| **Town Map** | Shows the region from above, with a scale | Remote sensing — and knowing how big things are, which is the inverse of monocular scale ambiguity ([`121`](answers/pokemon/121-high-resolution-tiling.md), [`139`](answers/pokemon/139-multimodal-safety-attacks.md), [`140`](answers/pokemon/140-multimodal-retrieval.md), [`175`](answers/pokemon/175-remote-sensing-imagery.md), [`176`](answers/pokemon/176-multimodal-agents-with-tools.md), [`192`](answers/pokemon/192-multimodal-monitoring.md)) |
| **Poké Flute** | Wakes the Snorlax. Played correctly or not at all | Audio generation's intolerance of approximation ([`173`](answers/pokemon/173-audio-and-music-generation.md), [`176`](answers/pokemon/176-multimodal-agents-with-tools.md), [`194`](answers/pokemon/194-on-device-translation.md)) |
| **Spotlight · Follow Me · Rage Powder** | Moves whose entire effect is deciding **where the attacks go** | **Attention**, and attention sinks ([`119`](answers/pokemon/119-cross-modal-attention.md)) |
| **Sandstorm** | Weather that chips every turn at anything not Rock, Ground or Steel | Difficult conditions, and slow accumulating damage ([`132`](answers/pokemon/132-quality-estimation.md), [`145`](answers/pokemon/145-gui-computer-use-agents.md), [`155`](answers/pokemon/155-sign-language-translation.md), [`157`](answers/pokemon/157-video-generation.md), [`181`](answers/pokemon/181-vision-language-action.md), [`183`](answers/pokemon/183-multimodal-accessibility.md), +1 more) |
| **Toxic** | Badly poisoned: 1/16, then 2/16, then 3/16, compounding | **Silent production drift.** Nothing looks wrong on the turn it lands ([`132`](answers/pokemon/132-quality-estimation.md), [`192`](answers/pokemon/192-multimodal-monitoring.md)) |
| **Confusion** | The Pokémon believes it is attacking and strikes itself | **Confidence that is not correctness** ([`132`](answers/pokemon/132-quality-estimation.md), [`182`](answers/pokemon/182-vlm-distillation.md), [`200`](answers/pokemon/200-what-actually-matters.md)) |
| **Transform** | Ditto's move. Copies everything except HP | Back-translation, and the watermark that survives ([`166`](answers/pokemon/166-watermarking-and-provenance.md)) |
| **Chatter** | Chatot's move; recorded a real voice | Voice cloning and consent ([`125`](answers/pokemon/125-speech-recognition.md), [`138`](answers/pokemon/138-speech-language-models.md), [`153`](answers/pokemon/153-speech-to-speech-translation.md)) |
| **Sketch** | Smeargle's move; copies one move permanently | A fixed-size summary of anything ([`112`](answers/pokemon/112-byte-level-and-tokenizer-free.md), [`117`](answers/pokemon/117-vision-language-architectures.md)) |
| **Hyper Training / Bottle Cap** | Maxes a stat's effect at level 100 without changing what it was born with | Papering over the original ([`149`](answers/pokemon/149-post-editing-human-in-the-loop.md)) |
| **Vs. Recorder / Battle Video** | Records a battle for replay, shareable by code | **Video.** A recording, watched turn by turn, with a text log beside it ([`126`](answers/pokemon/126-video-understanding.md), [`174`](answers/pokemon/174-egocentric-and-streaming-perception.md)) |
| **Regulation G** | A published competitive format: which Pokémon may enter, how many restricted ones | **A regulated domain.** You do not set the standard, your team is inspected, and afterwards you account for it ([`129`](answers/pokemon/129-mt-evaluation-beyond-bleu.md), [`187`](answers/pokemon/187-mt-in-regulated-domains.md)) |
| **The nickname limit** | Ten characters in the older games, raised to **twelve** in Generation VI | **Length-constrained generation.** The box was resized because the words outgrew it ([`144`](answers/pokemon/144-length-constrained-translation.md), [`150`](answers/pokemon/150-localisation-beyond-text.md)) |
| **FIGHT · BAG · POKéMON · RUN** | The battle menu | **An action space**, and the translator's working screen ([`145`](answers/pokemon/145-gui-computer-use-agents.md), [`188`](answers/pokemon/188-cat-tools-and-translator-workflow.md)) |
| **Ledges** | Kanto's ledges can be jumped down and not climbed back up | **Irreversibility.** Some of the world only moves one way, and no accuracy figure changes that ([`145`](answers/pokemon/145-gui-computer-use-agents.md), [`181`](answers/pokemon/181-vision-language-action.md), [`189`](answers/pokemon/189-multimodal-data-flywheel.md), [`199`](answers/pokemon/199-choosing-an-approach.md)) |
| **Focus Sash** | Survives a lethal hit on exactly **1 HP** | A system that is, by any average, alive ([`126`](answers/pokemon/126-video-understanding.md), [`132`](answers/pokemon/132-quality-estimation.md), [`181`](answers/pokemon/181-vision-language-action.md), [`187`](answers/pokemon/187-mt-in-regulated-domains.md)) |
| **Safety Goggles** | Block powder moves and weather damage — completely — and nothing else | **A narrow defence.** Real, total, and useless against the Voltorb on the floor ([`119`](answers/pokemon/119-cross-modal-attention.md), [`139`](answers/pokemon/139-multimodal-safety-attacks.md)) |
